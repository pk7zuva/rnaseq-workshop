...
#Arg2 output file name "rnaseq_workshop_fpkm_matrix.txt" 
...
sh ./Foo.sh | awk '{for (i=6; i<=NF; i=i+6) if(i==6) {printf ("%s\t%s\t%.1f\t",$1,$(i-3),$(i-1))} else if (i>6 && i<NF) {printf ("%.1f\t",$(i-1))}  else if (i==NF) {printf ("%.1f\n",$(i-1))}}' >> $2
